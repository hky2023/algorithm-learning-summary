...
sos算法更偏向于对于bit分组。实际上就是寻找两个数之间的1的个数差异来更新，确保在某一次bit加上它一次。枚举到bit，mask的实际意义是mask前bit位不同的超集之和。实际上一个超集和他的自己之间的何时加入，只取决于他们不同bit位最高的那一位，这种更新方式，还保证了路径唯一性，确保只可以加入超集一次。
